...
class AccVelMagLabelLoader(LabelReadWriter):
    ...
    @staticmethod
    def getName():
        return "accVelBlurMagScaled"

    def getNumLabel(self):
        return self.acc_dir_sampler.num_dirs + self.vel_dir_sampler.num_dirs + self.gaussFilterGen.N + 1 + 2
    ...
    def loadLabel(self, label_fileWOExt, ov_fps, target_fps, num_frames, frame_sampling_idx, startT, endT):
        ...
        accS_mag_labels = np.empty((0, num_frames), dtype=np.float32)
        if self.acc_dir_sampler is not None:
            ...
            accS_mag_labels = np.linalg.norm(accS_clip / 24.0, axis=1) 

        velS_clip_labels = np.empty((0, num_frames), dtype=np.float32)
        velS_mag_labels = np.empty((0, num_frames), dtype=np.float32)
        if self.vel_dir_sampler is not None:
            ...
            velS_mag_labels = np.linalg.norm(velS_clip, axis=1)
        ...
        labels = np.concatenate([torch.unsqueeze(eventfulness_label, 0),
                                 accS_clip_labels, velS_clip_labels, blurred_labels, 
                                 np.expand_dims(accS_mag_labels, axis=0), 
                                 np.expand_dims(velS_mag_labels, axis=0)])
