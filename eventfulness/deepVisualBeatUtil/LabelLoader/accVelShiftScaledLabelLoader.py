...
class AccVelShiftScaledLabelLoader(LabelReadWriter):
    ...
    def __init__(self, num_acc_dirs, num_vel_dirs, gaussFilterGen):
        ...
        print(f"Using {AccVelShiftScaledLabelLoader.getName()} data loader")
    ...
    @staticmethod
    def getName():
        return "beat_random_time_shift_scaled"
    ...
    def loadLabel(self, label_fileWOExt, ov_fps, target_fps, num_frames, frame_sampling_idx, startT, endT):
        ...
        clip_idx = np.logical_and(keyFrame_time_stamps >= startT - 1.5 / target_fps, keyFrame_time_stamps < endT)
        ...
        beat_idx = np.round((clip_time_stamps - startT) * target_fps + 1).astype(int)
        ...
        if self.acc_dir_sampler is not None:
            ...
            accS = np.concatenate([accS[1:accS.shape[0], :], np.zeros((min(max(0, len(accS) - 1), 1), accS.shape[1]))])
        ...
        if self.vel_dir_sampler is not None:
            ...
            velS = np.concatenate([velS[1:velS.shape[0], :], np.zeros((min(max(0, len(velS) - 1), 1), velS.shape[1]))])
