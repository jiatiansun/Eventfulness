...
class cAccVelShiftLeftScaledLabelLoader(LabelReadWriter):
    ...
    @staticmethod
    def getName():
        return "beat_random_time_c_shift_left_scaled"
    ...
    def loadLabel(self, label_fileWOExt, ov_fps, target_fps, num_frames, frame_sampling_idx, startT, endT):
        ...
        accCPos = self.readVecFromFile(label_fileWOExt, "_accCPos")[frame_sampling_idx, :2]
        accCPos = np.concatenate([accCPos[1:accCPos.shape[0], :], 
                                  np.zeros((min(max(0, len(accCPos) - 1), 1), accCPos.shape[1]))])
        accCPos = np.transpose(accCPos) / 24.0

        accCNeg = self.readVecFromFile(label_fileWOExt, "_accCNeg")[frame_sampling_idx, :2]
        accCNeg = np.concatenate([accCNeg[1:accCNeg.shape[0], :], 
                                  np.zeros((min(max(0, len(accCNeg) - 1), 1), accCNeg.shape[1]))])
        accCNeg = np.transpose(accCNeg) / 24.0
