...
class cAccVelShiftScaledSumabelLoader(LabelReadWriter):
    ...
    def __init__(self, num_acc_dirs, num_vel_dirs, gaussFilterGen):
        ...
        print(f"Using {cAccVelShiftScaledSumabelLoader.getName()} data loader")
    ...
    @staticmethod
    def getName():
        return "beat_random_time_c_sum_shift_scaled"
    ...
    def loadLabel(self, label_fileWOExt, ov_fps, target_fps, num_frames, frame_sampling_idx, startT, endT):
        ...
        if self.acc_dir_sampler is not None:
            accS_filename = label_fileWOExt + '_envelope_accC.csv'
        ...
        if self.vel_dir_sampler is not None:
            velS_filename = label_fileWOExt + '_envelope_velC.csv'
