...
class AccVelMmtDeltaScaledLabelLoader(LabelReadWriter):

    def __init__(self, gaussFilterGen):
        print(f"Using the accvel mmt rescaled data loader")
    ...
    def loadLabel(self, label_fileWOExt, ov_fps, target_fps, num_frames, frame_sampling_idx, startT, endT):
        ...
        accSPos = np.transpose(self.readVecFromFile(label_fileWOExt, "_accSPos")[frame_sampling_idx, :2]) / 24.0
        accSNeg = np.transpose(self.readVecFromFile(label_fileWOExt, "_accSNeg")[frame_sampling_idx, :2]) / 24.0
